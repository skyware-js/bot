type ArrayItem<R> = NonNullable<
	{ [K in keyof R]: R[K] extends Array<infer U> ? U : never }[keyof R]
>;
...
>(method: (cursor?: string | undefined, ...rest: Rest) => Promise<R>) {
	return async function*(
		...
		...rest: Rest
	): AsyncIterableIterator<ArrayItem<R>> {
		...
		do {
			...
			const [_key, items] =
				(Object.entries(page).find(([key, value]) =>
					key !== "cursor" && Array.isArray(value)
				) as [string, ArrayItem<R>[]] | undefined) ?? [];

			if (!items?.length) {
				throw new Error("Couldn't find results array in response");
			}
