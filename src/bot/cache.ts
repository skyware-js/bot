...
const DEFAULT_CACHE_OPTIONS: CacheOptions = { maxAge: 12 * 60 * 60, maxEntries: 250 };

export const makeCache = <T>(options?: CacheOptions | undefined) => {
	options = { ...DEFAULT_CACHE_OPTIONS, ...options };
	return new QuickLRU<string, T>({ maxAge: options.maxAge * 1000, maxSize: options.maxEntries });
};
